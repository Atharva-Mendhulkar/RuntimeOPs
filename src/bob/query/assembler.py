            ...
            for result in raw_results:
                try:
                    ...
                    evidence = Evidence(
                        file_path=result["file_path"],
                        line_range=(result["start_line"], result["end_line"]),
                        commit_sha=git_blame.commits[0]["commit_hash"] if git_blame.commits else None,
                        author=git_blame.commits[0]["author"] if git_blame.commits else None,
                        last_modified=git_blame.commits[0]["commit_date"] if git_blame.commits else None,
                    )
                    ...
                    metadata = {
                        ...
                        "dependency_count": file_metadata.get("imports_count", 0) + file_metadata.get("importers_count", 0),
        ...
        with GraphQuery() as graph_query:
            for result in results:
                try:
                    ...
                    result.metadata["total_dependencies"] = len(deps["upstream"]) + len(deps["downstream"])
