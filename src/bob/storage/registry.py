...
class IndexRegistry(Base):
    ...
    # Relationships
    access_grants = relationship("RepoAccess", back_populates="repository", cascade="all, delete-orphan")
...
class RepoAccess(Base):
    ...
    repo_id = Column(PGUUID(as_uuid=True), ForeignKey("index_registry.repo_id", ondelete="CASCADE"), nullable=False)
    ...
    def get_repository(self, repo_id: UUID) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "status": repo.status,
                "last_full_index": repo.last_full_index.isoformat() if repo.last_full_index else None,
                "last_incremental": repo.last_incremental.isoformat() if repo.last_incremental else None,
    ...
    def get_health_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            for status in ["idle", "indexing", "error", "stale"]:
                count = session.query(func.count(IndexRegistry.repo_id)).filter_by(status=status).scalar()
