...
class TestCompleteIngestionWorkflow:
    ...
    async def test_complete_repository_ingestion(
        ...
        result = test_weaviate.query.get(
            "CodeUnit",
...
class TestSemanticSearchWorkflow:
    ...
    async def test_semantic_search_accuracy(
        ...
        for test_case in test_cases:
            ...
            found = any(
                r["file_path"] == test_case["expected_file"] and
                r["symbol_name"] == test_case["expected_symbol"]
                for r in results
            )
            assert found, f"Expected {test_case['expected_symbol']} in {test_case['expected_file']} not found for query: {test_case['query']}"
