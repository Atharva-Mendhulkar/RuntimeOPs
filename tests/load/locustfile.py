...
SAMPLE_REPOS = ["test/sample-repo", "test/sample-repo-2", "test/large-repo"]
...
class BobUser(HttpUser):
    ...
    def semantic_search(self):
        ...
        with self.client.post(
            "/api/v1/search",
            json={"query": query, "repo_id": repo_id, "k": 10},
    ...
    def get_dependencies(self):
        ...
        with self.client.get(
            "/api/v1/dependencies",
            params={"file_path": file_path, "repo_id": repo_id, "hops": hops, "direction": "both"},
            headers=self.headers,
            catch_response=True,
...
ValueError: invalid value"""
        ...
        with self.client.post(
            ...
            json={"trace": stack_trace, "repo_id": repo_id},
            headers=self.headers,
            catch_response=True,
    ...
    def get_blast_radius(self):
        ...
        with self.client.post(
            "/api/v1/blast-radius",
            json={"files": files, "repo_id": repo_id},
            headers=self.headers,
            catch_response=True,
...
class BurstUser(HttpUser):
    ...
    def burst_requests(self):
        ...
        for _ in range(num_requests):
            ...
            self.client.post(
                "/api/v1/search",
                json={"query": query, "repo_id": repo_id, "k": 5},
                headers=self.headers,
...
def on_test_stop(environment, **kwargs):
    ...
    sorted_stats = sorted(stats.entries.values(), key=lambda x: x.num_requests, reverse=True)[:5]

    for stat in sorted_stats:
        print(
            f"  {stat.name}: {stat.num_requests} requests, "
