...
class TestQueryPerformance:
    ...
    async def test_semantic_search_performance(
        ...
        for query in queries:
            ...
            response = await test_client.post(
                "/api/v1/search",
                json={"query": query, "repo_id": large_repository["repo_id"], "k": 10},
                headers=auth_headers,
            )
    ...
    async def test_graph_query_performance(
        ...
        for test_case in test_cases:
            ...
            response = await test_client.get(
                "/api/v1/dependencies", params=test_case["params"], headers=auth_headers
            )
            latency = time.time() - start
    ...
    async def test_file_retrieval_performance(
        ...
        response = await test_client.get(
            "/api/v1/file",
            params={"file_path": file_path, "repo_id": large_repository["repo_id"]},
            headers=auth_headers,
        )
    ...
    async def test_repository_ingestion_performance(
        ...
        for case in test_cases:
            ...
            repo = self._generate_test_repository(
                num_files=case["files"], repo_name=f"perf-test-{case['name']}"
            )
            ...
            performance_tracker.record(
                "ingestion", duration, files=case["files"], throughput=throughput
            )

            assert (
                duration < case["max_time"]
...
class TestConcurrentPerformance:
    ...
    async def test_concurrent_search_requests(
        ...
        async def make_search_request():
            ...
            response = await test_client.post(
                "/api/v1/search",
                json={"query": "test function", "repo_id": large_repository["repo_id"], "k": 10},
                headers=auth_headers,
            )
            latency = time.time() - start
    ...
    async def test_mixed_workload_performance(
        ...
        async def make_file_request():
            return await test_client.get(
                "/api/v1/file",
                params={"file_path": "src/module_0.py", "repo_id": large_repository["repo_id"]},
                headers=auth_headers,
            )
    ...
    async def test_repository_count_scalability(
        ...
        for count in repo_counts:
            ...
            for i in range(count):
                repo = self._generate_test_repository(num_files=50, repo_name=f"scale-test-{i}")
                response = await test_client.post(
                    "/api/v1/repositories/ingest",
                    json={"repo_id": repo["repo_id"]},
                    headers=auth_headers,
            ...
            start = time.time()
            response = await test_client.post(
                "/api/v1/search", json={"query": "test function", "k": 10}, headers=auth_headers
            )
            latency = time.time() - start
