...
class TestAuthentication:
    ...
    async def test_invalid_token_rejected(self, test_client):
        ...
        response = await test_client.get(
            "/api/v1/search", headers={"Authorization": "Bearer invalid_token_12345"}
        )
        assert response.status_code == 401
    ...
    async def test_expired_token_rejected(self, test_client):
        ...
        expired_token = create_access_token(
            data={"sub": "test_user"}, expires_delta=timedelta(seconds=-1)  # Already expired
        )

        response = await test_client.get(
            "/api/v1/search", headers={"Authorization": f"Bearer {expired_token}"}
        )
        assert response.status_code == 401
    ...
    async def test_tampered_token_rejected(self, test_client):
        ...
        valid_token = create_access_token(
            data={"sub": "test_user"}, expires_delta=timedelta(hours=1)
        )
        ...
        response = await test_client.get(
            "/api/v1/search", headers={"Authorization": f"Bearer {tampered_token}"}
        )
        assert response.status_code == 401
    ...
    async def test_malformed_auth_header(self, test_client):
        ...
        for headers in malformed_headers:
            response = await test_client.get("/api/v1/search", headers=headers)
            assert response.status_code == 401
    ...
    async def test_token_with_invalid_signature(self, test_client):
        ...
        response = await test_client.get(
            "/api/v1/search", headers={"Authorization": f"Bearer {fake_token}"}
        )
        assert response.status_code == 401
...
class TestAuthorization:
    ...
    async def test_read_only_user_cannot_write(self, test_client, limited_auth_headers):
        ...
        for endpoint, method, data in write_operations:
            if method == "POST":
                response = await test_client.post(endpoint, json=data, headers=limited_auth_headers)
            elif method == "DELETE":
                response = await test_client.delete(
                    endpoint, params=data, headers=limited_auth_headers
                )
    ...
    async def test_cross_repository_access_denied(self, test_client, auth_headers):
        ...
        response = await test_client.post(
            "/api/v1/search",
            json={"query": "test", "repo_id": "unauthorized/repo"},
...
class TestSQLInjection:
    ...
    async def test_sql_injection_in_search_query(
        self, test_client, auth_headers, sql_injection_payloads
    ):
        """Test SQL injection attempts in search queries"""
        for payload in sql_injection_payloads:
            response = await test_client.post(
                "/api/v1/search",
                json={"query": payload, "repo_id": "test/repo", "k": 10},
                headers=auth_headers,
            )
    ...
    async def test_sql_injection_in_repo_id(
        ...
        for payload in sql_injection_payloads:
            response = await test_client.post(
                "/api/v1/search",
                json={"query": "test", "repo_id": payload, "k": 10},
                headers=auth_headers,
            )
    ...
    async def test_sql_injection_in_file_path(
        ...
        for payload in sql_injection_payloads:
            response = await test_client.get(
                "/api/v1/file",
                params={"file_path": payload, "repo_id": "test/repo"},
                headers=auth_headers,
            )
            # Should reject or sanitize
            assert response.status_code in [200, 400, 404]
...
class TestPathTraversal:
    ...
    async def test_path_traversal_in_file_path(
        self, test_client, auth_headers, path_traversal_payloads
    ):
...
class TestXSS:
    ...
    async def test_xss_in_search_query(self, test_client, auth_headers, xss_payloads):
    ...
    async def test_xss_in_error_messages(self, test_client, auth_headers):
        ...
        response = await test_client.get(
            "/api/v1/file",
            params={"file_path": xss_payload, "repo_id": "test/repo"},
            headers=auth_headers,
        )
...
class TestInputValidation:
    ...
    async def test_oversized_query_rejected(self, test_client, auth_headers):
        ...
        response = await test_client.post(
            "/api/v1/search",
            json={"query": oversized_query, "repo_id": "test/repo", "k": 10},
            headers=auth_headers,
        )
    ...
    async def test_invalid_k_parameter(self, test_client, auth_headers):
        ...
        for k in invalid_k_values:
            response = await test_client.post(
                "/api/v1/search",
                json={"query": "test", "repo_id": "test/repo", "k": k},
                headers=auth_headers,
            )
            assert response.status_code == 400
    ...
    async def test_invalid_hops_parameter(self, test_client, auth_headers):
        ...
        for hops in invalid_hops:
            response = await test_client.get(
                "/api/v1/dependencies",
                params={"file_path": "test.py", "repo_id": "test/repo", "hops": hops},
                headers=auth_headers,
            )
            assert response.status_code == 400
    ...
    async def test_empty_required_fields(self, test_client, auth_headers):
        ...
        for endpoint, data in test_cases:
            if "file_path" in data:
                response = await test_client.get(endpoint, params=data, headers=auth_headers)
            else:
                response = await test_client.post(endpoint, json=data, headers=auth_headers)
            assert response.status_code == 400
...
class TestCORS:
    ...
    async def test_cors_headers_present(self, test_client):
        ...
        response = await test_client.options(
            "/api/v1/search", headers={"Origin": "https://example.com"}
        )
    ...
    async def test_cors_restricts_origins(self, test_client):
        ...
        response = await test_client.options(
            "/api/v1/search", headers={"Origin": "https://malicious-site.com"}
        )
...
class TestEncryption:
    ...
    async def test_sensitive_data_not_in_logs(self, test_client, auth_headers):
        ...
        response = await test_client.post(
            "/api/v1/search",
            json={"query": "password123", "repo_id": "test/repo", "k": 10},  # Sensitive data
            headers=auth_headers,
        )
    ...
    async def test_tokens_not_in_response(self, test_client, auth_headers):
        ...
        response = await test_client.post(
            "/api/v1/search",
            json={"query": "test", "repo_id": "test/repo", "k": 10},
            headers=auth_headers,
        )
...
class TestSecurityHeaders:
    ...
    async def test_security_headers_present(self, test_client, auth_headers):
        ...
        response = await test_client.get("/health/ready", headers=auth_headers)
